...
export function TopNav() {
  ...
  return (
    <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      ...
      <nav className="w-full rounded-[18px] border border-[rgba(255,241,191,0.22)] bg-[rgba(67,40,24,0.14)] p-2 shadow-[0_12px_28px_rgba(67,40,24,0.12)] backdrop-blur-[6px] sm:w-[75%] sm:max-w-[690px] sm:rounded-[20px] sm:p-1">
        <div className="grid grid-cols-6 gap-2 sm:grid-cols-5 sm:gap-3">
          {items.map((item, index) => {
            const active = pathname === item.href;
            const mobileSpan = index < 3 ? "col-span-2" : "col-span-3";

            return (
              <Link
                key={item.href}
                href={item.href}
                className={`${mobileSpan} flex min-h-[54px] items-center justify-center rounded-[14px] border px-3 text-center text-[13px] font-semibold leading-tight tracking-[-0.02em] shadow-[0_6px_18px_rgba(67,40,24,0.08)] transition sm:col-span-1 sm:min-h-[60px] sm:px-4 ${
                  active
