          ...
          <SectionCard title={`Receipt #${receipt.id}`} description={receipt.storeName || "Stored receipt record"}>
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <dd className="mt-1 text-base font-medium">{formatCurrency(receipt.subtotal, receipt.currency)}</dd>
              </div>
              <div>
                <dt className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">Tax</dt>
                <dd className="mt-1 text-base font-medium">{formatCurrency(receipt.tax, receipt.currency)}</dd>
              </div>
              <div>
                <dt className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">Total</dt>
                <dd className="mt-1 text-base font-medium">{formatCurrency(receipt.total, receipt.currency)}</dd>
              </div>
              <div>
                  ...
                  <tbody>
                    {items.map((item, index) => (
                      <tr key={item.id} className="rounded-[14px] bg-[var(--surface-soft)] text-[var(--text)]">
                        ...
                        <td className="px-3 py-3">{formatCurrency(item.unitPrice, receipt.currency)}</td>
                        <td className="px-3 py-3">{formatCurrency(item.lineTotal, receipt.currency)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
